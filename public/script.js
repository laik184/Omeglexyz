...
async function createOffer() {
    try {
        ...
        // Check if we're already in a negotiation state
        if (peerConnection.signalingState !== 'stable') {
            console.log('Cannot create offer, peer connection not in stable state:', peerConnection.signalingState);
            return;
        }
        ...
        console.log('Offer created, checking signaling state before setting local description');
        ...
        if (peerConnection && peerConnection.signalingState !== 'stable') {
            try {
                console.log('Resetting peer connection state after offer creation error');
...
async function handleWebRTCOffer(data) {
    try {
        ...
        if (currentState === 'have-local-offer') {
            ...
            // Implement collision resolution using description comparison
            const localOffer = peerConnection.localDescription;
            const remoteOffer = new RTCSessionDescription(data.offer);
            
            // Compare offers to determine which peer should be "polite" (rollback)
            // Use a simple comparison based on offer SDP string length or other criteria
            const shouldRollback = localOffer.sdp.length < remoteOffer.sdp.length || 
                                   (localOffer.sdp.length === remoteOffer.sdp.length && Math.random() < 0.5);
            ...
            if (shouldRollback) {
                ...
                // Reset state for accepting remote offer
                isRemoteDescriptionSet = false;
                ...
                console.log('Ignoring remote offer due to collision resolution (keeping local offer)');

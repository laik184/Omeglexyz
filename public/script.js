...
// Enhanced WebRTC connection failure handling with detailed logging and improved retry logic
function handleWebRTCConnectionFailure() {
    const failureTimestamp = new Date().toISOString();
    
    console.log(`❌ WebRTC connection failed at ${failureTimestamp}`);
    console.log(`🔄 Retry attempt: ${webrtcRetryAttempts}/${MAX_WEBRTC_RETRIES}`);
    ...
    if (webrtcRetryAttempts < MAX_WEBRTC_RETRIES) {
        webrtcRetryAttempts++;
        const retryDelay = Math.min(2000 * Math.pow(2, webrtcRetryAttempts - 1), 10000); // Exponential backoff with cap
        
        console.log(`🔄 Attempting WebRTC retry ${webrtcRetryAttempts}/${MAX_WEBRTC_RETRIES} in ${retryDelay}ms`);
        
        addVideoSystemMessage(`Connection failed. Retrying... (${webrtcRetryAttempts}/${MAX_WEBRTC_RETRIES})`);
        showErrorFeedback(`Connection failed. Attempting retry ${webrtcRetryAttempts}/${MAX_WEBRTC_RETRIES} in ${retryDelay/1000}s`);
        
        setTimeout(async () => {
            try {
                console.log('🔄 Starting WebRTC retry process...');
                
                // Check if peer connection still exists and is in failed state
                if (peerConnection && peerConnection.connectionState === 'failed') {
                    ...
                    // Clear ICE candidate queue before restart
                    const clearedCandidates = iceCandidateQueue.length;
                    iceCandidateQueue = [];
                    isRemoteDescriptionSet = false;
                    
                    console.log(`🧹 Cleared ${clearedCandidates} queued candidates before ICE restart`);
                    ...
                    peerConnectionTimeout = setTimeout(() => {
                        if (peerConnection && peerConnection.connectionState !== 'connected') {
                            console.log('⏰ Retry attempt timed out');
                            handleWebRTCConnectionFailure();
                        }
                    }, PEER_CONNECTION_TIMEOUT);
                    
                } else {
                    console.log('🔄 Peer connection not in failed state, creating new connection...');
                ...
                console.log('✅ WebRTC retry process initiated successfully');
                
            } catch (retryError) {
                console.error('❌ Error during WebRTC retry:', retryError);
                addVideoSystemMessage(`Retry failed: ${retryError.message}`);
        ...
        console.log('❌ Maximum WebRTC retry attempts reached, showing permanent failure');
        console.log('📊 Final failure state:', {
            totalRetries: webrtcRetryAttempts,
            finalSignalingState: peerConnection?.signalingState,
            finalIceConnectionState: peerConnection?.iceConnectionState,
            finalConnectionState: peerConnection?.connectionState,
            finalQueueLength: iceCandidateQueue.length
        });
        
        addVideoSystemMessage('Video call failed after multiple attempts. Connection could not be established.');
        showErrorFeedback('Video connection failed permanently. Please check your network and try again.');
        
        // Show enhanced retry button with diagnostic info
        const retryBtn = document.createElement('button');
        ...
        retryBtn.onmouseover = () => retryBtn.style.backgroundColor = '#c82333';
        retryBtn.onmouseout = () => retryBtn.style.backgroundColor = '#dc3545';

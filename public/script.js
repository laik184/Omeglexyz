    ...
    if ('serviceWorker' in navigator && 'caches' in window) {
        ...
        window.addEventListener('load', () => {
            ...
            .catch((error) => {
                ...
                showUserFeedback('App offline features unavailable', 'warning');
            });
        });
        ...
        showUserFeedback('Offline features not supported in this browser', 'warning');
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Setup mobile-specific input handlers
...
// Enhanced resize handler for mobile keyboard detection
function handleMobileViewportChange() {
    const currentHeight = window.innerHeight;
    const heightDifference = initialViewportHeight - currentHeight;
    
    // Detect keyboard showing/hiding (threshold of 150px for keyboard detection)
    ...
    if (keyboardVisible && !wasKeyboardVisible) {
        // Keyboard appeared - adjust chat containers immediately
        adjustChatContainersForKeyboard(true);
        
        // Scroll to latest message after keyboard animation completes
        ...
        // Keyboard hidden - restore normal layout
        adjustChatContainersForKeyboard(false);
        
        // Scroll to latest message after layout restoration
        scrollTimeoutId = setTimeout(() => {
            if (currentChatType === 'text' && chatMessages) {
                scrollToLatestMessage(chatMessages, true);
            } else if (currentChatType === 'video' && videoChatMessages) {
...
// Adjust chat containers for mobile keyboard
function adjustChatContainersForKeyboard(keyboardShown) {
    ...
    if (keyboardShown) {
        ...
        if (chatMessages) {
            chatMessages.style.maxHeight = `${window.innerHeight - 160}px`;
            chatMessages.style.paddingBottom = '20px';
        }
        
        if (videoChatMessages) {
            videoChatMessages.style.maxHeight = `${window.innerHeight * 0.4}px`;
            videoChatMessages.style.paddingBottom = '20px';
        }
        
        // Ensure input containers stay visible
        ...
        if (videoChatInputContainer) {
            ...
            videoChatInputContainer.style.zIndex = '10000';
        }
...
// Enhanced input focus handlers for mobile
function setupMobileInputHandlers() {
    const messageInput = document.getElementById('messageInput');
    const videoMessageInput = document.getElementById('videoMessageInput');
    
    // Text chat input handlers
    if (messageInput) {
    ...
    // Video chat input handlers
    if (videoMessageInput) {
        videoMessageInput.addEventListener('focus', () => {
...
async function initializeWebRTC(initiator) {
    try {
        ...
        peerConnection.onicecandidate = (event) => {
            if (event.candidate) {
                ...
                if (event.candidate.type === 'relay') {
                    ...
                    console.log('📡 TURN relay candidate detected - NAT traversal via relay');
                }
                ...
                if (socket?.readyState === WebSocket.OPEN) {
                    ...
                    console.warn('⚠️ Cannot send ICE candidate - WebSocket not open');
                }
            } else {
        ...
        peerConnection.onconnectionstatechange = () => {
            ...
            webrtcDebugger.logStateTransition('connection', oldState, peerConnection.connectionState, {
                iceConnectionState: peerConnection.iceConnectionState,
                signalingState: peerConnection.signalingState,
            ...
            switch (peerConnection.connectionState) {
                ...
                case 'connected':
                    ...
                    // Log connection success metrics
                ...
                case 'disconnected':
                    ...
                    console.log('📊 Disconnection Analysis:', disconnectAnalysis);
    ...
    if (videoConnectBtn) {
        videoConnectBtn.addEventListener('click', () => {
            ...
            if (label && label.textContent === "New") {
                ...
                console.log('👆 Video chat Skip button clicked - starting comprehensive disconnect process...');
                addVideoSystemMessage("You disconnected from the stranger.");
                
                // Perform immediate video cleanup before general disconnect
                performVideoCleanup('skip_button_clicked');
                
                // Wait a moment to ensure video cleanup completes before general cleanup
                setTimeout(() => {
                    cleanupConnections();
                }, 100);
            }
...
function endVideoCall() {
    ...
    if (document.body.classList.contains('mobile-video-active')) {
        document.body.classList.remove('mobile-video-active');
        console.log('✅ Removed mobile-video-active class from body');
    }

    // Perform comprehensive cleanup
    performVideoCleanup('end_video_call');
    cleanupWebRTCConnection('end_video_call');

    ...
    try {
        ...
        peerConnection.ontrack = (event) => {
            ...
            if (event.streams && event.streams.length > 0) {
                ...
                // Browser compatibility fallback for metadata loading (Safari/Firefox)
                remoteVideo.onloadedmetadata = () => {
                    console.log('Remote video metadata loaded');
                    remoteVideo.play().catch((e) => {
                        console.error("Remote video autoplay failed in onloadedmetadata:", e);
                        // Show user interaction prompt for autoplay policy
                        addVideoSystemMessage('Click anywhere to enable remote video playback');
                    });
                };
                
                // Timed fallback for autoplay issues (cross-browser compatibility)
                setTimeout(() => {
                    if (remoteVideo.paused && remoteVideo.readyState >= 2) {
                        console.log('Attempting fallback play after timeout');
                        remoteVideo.play().catch((e) => {
                            console.error("Fallback play failed after timeout:", e);
                            addVideoSystemMessage('Remote video requires manual play - click the video area');
                        });
                    }
                }, 1000);
                
                // Enhanced play handling with detailed logging
                const handleVideoPlay = async () => {
                    try {
                        console.log('Attempting to play remote video...');
                        console.log('Remote video readyState:', remoteVideo.readyState);
                        console.log('Remote video networkState:', remoteVideo.networkState);
                        ...
                        // Try user interaction workaround for autoplay policy
                ...
                // Wait for sufficient metadata before playing
                if (remoteVideo.readyState >= 2) { // HAVE_CURRENT_DATA or higher
                    console.log('Video has sufficient data, playing immediately');
                    handleVideoPlay();
                } else {
                    ...
                    setTimeout(() => {
                        if (remoteVideo.readyState < 2) {
                            console.log('Timeout waiting for video data, attempting play anyway');
                            handleVideoPlay();
                        }
                    }, 3000);
                }
